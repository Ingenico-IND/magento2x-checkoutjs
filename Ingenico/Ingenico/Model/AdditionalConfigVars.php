...
namespace Ingenico\Ingenico\Model;

use \Magento\Checkout\Model\ConfigProviderInterface;
use Ingenico\Ingenico\Helper\Data as Config;
...
class AdditionalConfigVars implements ConfigProviderInterface
{
    ...
    {
        return $this->config->getConfig('payment/ingenicopayment/ingenico_mercode');
    }

    private function embedPaymentGatewayOnPage()
    {
        return $this->config->getConfig('payment/ingenicopayment/embedPaymentGatewayOnPage') ? true : false;
    }

    private function displayErrorMessageOnPopup()
    {
        return $this->config->getConfig('payment/ingenicopayment/displayErrorMessageOnPopup') ? true : false;
    }

    private function getSchemeCode()
    {
        return $this->config->getConfig('payment/ingenicopayment/ingenico_scode');
    }
    ...
    {
        return $this->config->getConfig('payment/ingenicopayment/ingenico_key');
    }
    ...
    private function getAmount()
    {
        ...
        $orderId = $checkout_session->getLastRealOrderId();
        $order = $sales_order->loadByIncrementId($orderId);
        if ($this->config->getConfig('payment/ingenicopayment/webservice_locator') == 'test') {
            $Amount = "1.0";
        } else {
    ...
    {
        if ($this->config->getConfig('payment/ingenicopayment/primary_color_code')) {
            $primary_color_code = $this->config->getConfig('payment/ingenicopayment/primary_color_code');
        } else {
    ...
    {
        if ($this->config->getConfig('payment/ingenicopayment/secondary_color_code')) {
            $secondary_color_code = $this->config->getConfig('payment/ingenicopayment/secondary_color_code');
        } else {
    ...
    {
        if ($this->config->getConfig('payment/ingenicopayment/button_color_code_1')) {
            $button_color_code_1 = $this->config->getConfig('payment/ingenicopayment/button_color_code_1');
        } else {
    ...
    {
        if ($this->config->getConfig('payment/ingenicopayment/button_color_code_2')) {
            $button_color_code_2 = $this->config->getConfig('payment/ingenicopayment/button_color_code_2');
        } else {
    ...
    {
        $logo_url = $this->config->getConfig('payment/ingenicopayment/merchant_logo_url');
    ...
    {
        return $this->config->getConfig('payment/ingenicopayment/showLoader') ? true : false;
    }

    private function getMerchantMsg()
    {
        $merchantMsg = $this->config->getConfig('payment/ingenicopayment/merchantMsg');
    ...
    {
        $disclaimerMsg = $this->config->getConfig('payment/ingenicopayment/disclaimerMsg');
    ...
    {
        return $this->config->getConfig('payment/ingenicopayment/enableExpressPay') ? true : false;
    }

    private function getseparateCardMode()
    {
        return $this->config->getConfig('payment/ingenicopayment/separateCardMode') ? true : false;
    }

    private function getenableNewWindowFlow()
    {
        return $this->config->getConfig('payment/ingenicopayment/enableNewWindowFlow') ? true : false;
    }

    private function getpaymentorderarray()
    {
        $paymentModeArray = array();
        if ($this->config->getConfig('payment/ingenicopayment/paymentModeOrder')) {
            $paymentModeOrder = $this->config->getConfig('payment/ingenicopayment/paymentModeOrder');
    ...
    {
        $paymentmodes = $this->config->getConfig('payment/ingenicopayment/paymentMode');
    ...
    {
        return $this->config->getConfig('payment/ingenicopayment/txnType');
    }

    private function getenableInstrumentDeRegistration()
    {
        if ($this->getenableExpressPay() && $this->config->getConfig('payment/ingenicopayment/enableInstrumentDeRegistration')) {
            return true;
        } else {
            return false;
        }
    ...
    {
        if ($this->getenableExpressPay() && $this->config->getConfig('payment/ingenicopayment/hideSavedInstruments')) {
            return true;
        } else {
            return false;
        }
    ...
    {
        return $this->config->getConfig('payment/ingenicopayment/saveInstrument') ? true : false;
    }
}
